...
class helpCommand(sublime_plugin.TextCommand):
  def run(self, edit):   
    for region in self.view.sel():
        ...
        while (flag == 'true'):  
          ...
          for region in self.view.sel():
            ...
            if len(h1text) > 0:        
             if h1text[0] == ' ' or h1text[0] == '@' or h1text[0] == '\t' or h1text[0] == '[' or h1text[0] == ']':
               flag = 'false'
               if h1text[0] == '[' or h1text[0] == ']':
                self.view.sel().clear() 
               else:
                self.view.sel().clear() 
                fend = fend + 1
    ...
    if len(h1text) > 0 and h1text[0] == '[':
      newStr = h1text.replace("[","")  
      newStr = newStr.replace("]","")  
      if newStr == 'em' or newStr == 'strong' or newStr == 'dfn' or newStr == 'code' or newStr == 'samp' or newStr == 'kbd' or newStr == 'var':
        webbrowser.open_new("http://www.w3schools.com/tags/tag_phrase_elements.asp") 
        return 
      else:
        webbrowser.open_new("http://www.w3schools.com/tags/tag_"+newStr+".asp")
        return
    elif h1text == 'dsp':
